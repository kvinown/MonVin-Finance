...
export const LoginPage: React.FC = () => {
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		setIsLoading(true);
		try {
			// Panggil Firebase Service (Bisa pakai email atau username)
			...
			if (res.success && res.user) {
				...
				// Siapkan data user (username kosong dulu, nanti diisi otomatis oleh AuthContext di background)
				const userData = {
					id: res.user.uid,
					name: res.user.displayName || "Pengguna",
					email: res.user.email || "",
					username: identifier, // Sementara
				};
	...
	const handleGoogleAuth = async () => {
		...
		try {
			...
			if (res.success && res.user) {
				// Sinkronkan ke Context kita
				const token = await res.user.getIdToken();
				login(token, {
					id: res.user.uid,
					name: res.user.displayName || "Pengguna",
					email: res.user.email || "",
					username: "", // Akan disinkronkan otomatis oleh AuthContext di background
				});
				navigate("/dashboard");
			} else {
			...
			setErrorMsg("Terjadi kesalahan saat memuat akun Google.");
		} finally {
			setIsLoading(false);
		}
	};
