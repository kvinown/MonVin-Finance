...
export const DashboardPage: React.FC = () => {
	...
	useEffect(() => {
		const fetchDashboard = async () => {
			try {
				...
				setErrorMsg(err.message || "Gagal memuat data dasbor.");
			} finally {
	...
	const handleFetchInsight = async () => {
		...
		try {
			...
			setInsight("Gagal menganalisis data keuangan saat ini. Silakan coba lagi nanti.");
		} finally {
	...
	return (
		<div className="space-y-8 max-w-7xl mx-auto">
			...
			<div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden transition-colors duration-300">
				...
				{/* VIEW DESKTOP: Render Tabel Standar (Disembunyikan di Mobile) */}
							...
							{data?.recentTransactions && data.recentTransactions.length > 0 ? (
								data.recentTransactions.map((trx) => (
									<tr
										...
										<td className="p-4 text-slate-500">{trx.wallet.name}</td>
				...
				{/* VIEW MOBILE: Render List Item Vertikal (Disembunyikan di Desktop) */}
